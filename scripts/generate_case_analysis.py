...
LABEL_FILE = Path("docs/label/confirmed_fraud_customer.txt")
OUT_MD = Path("docs/analytics/confirmed_fraud_customer_case_analysis.md")
OUT_JSON = Path("data/confirmed_fraud_case_details.json")
HOST = "172.16.100.213"
PORT = 9666
USER = "data-analyst"
CATALOG = "hive"
...
def run():
    ...
    # Staging tables (latest snapshot)
        ...
        json.dumps(
            {
                ...
                "customers": customers,
                "transactions": txns,
                "bank_accts": bank_accts,
                "coll_accts": coll_accts,
                "ent_realnames": ent_realnames,
                "ref_persons": ref_persons,
                "person_realnames": person_realnames,
    ...
    for cid in unique_ids:
        ...
        acct_names = set(r.get("acct_name") or "" for r in cba)
    ...
    lines.append("  - 注：用户指定的 `/docs/reference/label/confirmed_fraud_customer.txt` 在当前仓库中不存在，已自动定位到实际文件 `docs/label/confirmed_fraud_customer.txt`。")
    ...
    # Reuse summary
    reused_bank_names = defaultdict(set)
    for r in bank_accts:
        no = str(r.get("acct_no") or "").strip()
        name = str(r.get("acct_name") or "").strip() or "(空)"
        if no:
            reused_bank_names[no].add(name)
    reused_coll_names = defaultdict(set)
    for r in coll_accts:
        no = str(r.get("bank_acct_no") or "").strip()
        name = str(r.get("bank_acct_name") or "").strip() or "(空)"
        if no:
            reused_coll_names[no].add(name)
    if reused_bank_nos:
        ...
        for no, custs in sorted(reused_bank_nos.items(), key=lambda x: -len(x[1])):
            acct_names_str = " / ".join(sorted(reused_bank_names.get(no, {"(空)"})))
