...
| creditor_country | varchar(10) | COALESCE(pd.BANK_COUNTRY, pd.COLL_COUNTRY_CD) | Payee country (bank country first) |
...
| fund_purpose | varchar(200) | COALESCE(NULLIF(pd.FUND_PURPOSE,''), NULLIF(po.FUND_PURPOSE,''), NULLIF(po.PAY_PURPOSE,'')) | Fund purpose (fallback to pay_purpose) |
