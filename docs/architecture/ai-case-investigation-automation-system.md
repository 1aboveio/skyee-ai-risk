...
| 交付类别 | 使用者 | 覆盖范围 |
| --- | --- | --- |
| 案件流程系统平台 | 业务用户间接使用，流程/系统管理员配置 | 承载案件接入、案件管理与 Workflow、状态流转、SLA、处置、后续任务和审计回写 |
...
客户视角下，`F1` 和 `F2` 是大模块；图中的来源适配、接入处理、案件池、Workflow 和 Task Queue 是它们的内部子能力，不单独作为商务 scope 拆分。

- F1 商户案件接入模块：承接 KYC、规则、实地核查、举报、专项、RFI、离线规则、图谱关联等来源，并完成触发归一、标准化、去重、归并和商户建案。
- F2 案件管理与 Workflow 模块：包含案件池、Workflow 编排层和 Workflow Task Queue，承载优先级、SLA、队列、分派入口、状态流转、节点审批和可执行任务队列。
- F3 处置流转模块：承载 ACCEPT、REJECT、ESCALATION 的系统流转、权限和审计。
- F4 后续任务模块：承载 RFI 回应、策略反馈等后续任务。
- F5 Workflow 回写：回写状态关闭、SLA 和审计记录。
- F6 关联图谱触发：作为内部触发的一类，将商户、账户、交易、渠道和历史风险之间的关联命中转成案件流入信号。
...
- 关联图谱：当前缺少图谱数据结构、关系类型、命中规则、输出格式和图谱触发进入商户案件接入模块的标准。
...
- 案件流程系统平台编排任务和主状态，承载案件接入、案件管理与 Workflow、图谱关联触发、处置流转、后续任务和审计回写。
...
**商户案件接入模块**面向上游来源，将 KYC、规则触发、实地核查、内部举报、专项盘点、RFI、离线规则、图谱关联等不同来源转换成统一的案件流入事件，并负责校验、标准化、去重、归并、商户建案，或把新的风险信号挂载到已有案件上。

在系统模块承载图中，KYC 准入与重检、交易规则预警、实地核查被标注为客户原流程中的已有来源；商户案件接入模块不是新的业务来源，而是承接这些来源并补齐其他来源接入能力的系统模块。

实现上，商户案件接入模块可以拆成“来源适配”和“商户建案处理”两个子能力，但客户视角下它们属于同一个交付模块。

### 6.2 案件池与任务队列

**案件管理与 Workflow 模块**包含案件池、Workflow 编排层和 Workflow Task Queue。客户视角下，它们共同解决“案件如何排队、如何流转、如何分派任务、如何跟踪 SLA”的问题。

案件池是案件级视图，用于查看案件主体、来源、风险信号、优先级、SLA、主状态、当前任务和负责人。

**关联图谱触发**属于内部触发的一类，不是案件池之后的主流程节点。它将商户、账户、交易、渠道、黑名单、历史风险记录和关联坏人之间的关系命中，转换成标准化案件流入事件，再进入商户案件接入模块。
...
### 8.2 来源适配和商户建案处理是一个东西吗？

客户视角下，是同一个 `F1 商户案件接入模块`。

工程实现上可以拆成两个子能力：来源适配负责对接不同上游来源，并输出统一的案件流入事件；商户建案处理负责把案件流入事件变成案件，或把新风险信号挂载到已有案件上。

推荐对外口径：作为一个模块报价和沟通；实现方案里再说明内部包含来源适配、标准化、去重、归并和商户建案。
