"""
Sync cust_store_info from MySQL to Hudi via Spark Connect.
"""
import sys
import os
...
class StgCustStoreInfoEtl(MySqlEtl):
    src_db = "usr_skyee_mw"
    src_tbl = "cust_store_info"
    dst_db = "usr_skyee_mw"
    dst_tbl = "stg_cust_store_info"
    path = "/user/hive/warehouse/usr_skyee_mw.db/stg_cust_store_info"
    id = "ID"
    ts = "LST_UPD_TIME"
...
def main(
    ...
    etl = StgCustStoreInfoEtl(url=url, start_date=start_date, end_date=end_date, bulk=bulk)
    etl.spark = spark
    etl()
    spark.stop()
