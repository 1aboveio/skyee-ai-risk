"""
Sync cust_foreign_trade_order_logistics from MySQL to Hudi via Spark Connect.
"""
import sys
import os
...
class StgCustForeignTradeOrderLogisticsEtl(MySqlEtl):
    src_db = "usr_skyee_mw"
    src_tbl = "cust_foreign_trade_order_logistics"
    dst_db = "usr_skyee_mw"
    dst_tbl = "stg_cust_foreign_trade_order_logistics"
    path = "/user/hive/warehouse/usr_skyee_mw.db/stg_cust_foreign_trade_order_logistics"
    id = "ID"
    ts = "LST_UPD_TIME"
...
def main(
    ...
    etl = StgCustForeignTradeOrderLogisticsEtl(url=url, start_date=start_date, end_date=end_date, bulk=bulk)
    etl.spark = spark
    etl()
    spark.stop()
