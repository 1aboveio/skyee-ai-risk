"""
Sync cust_realname_enterprise_ref_person from MySQL to Hudi via Spark Connect.
"""
import sys
import os
...
class StgCustRealnameEnterpriseRefPersonEtl(MySqlEtl):
    src_db = "usr_skyee_mw"
    src_tbl = "cust_realname_enterprise_ref_person"
    dst_db = "usr_skyee_mw"
    dst_tbl = "stg_cust_realname_enterprise_ref_person"
    path = "/user/hive/warehouse/usr_skyee_mw.db/stg_cust_realname_enterprise_ref_person"
    id = "ID"
    ts = "LST_UPD_TIME"
...
def main(
    ...
    etl = StgCustRealnameEnterpriseRefPersonEtl(url=url, start_date=start_date, end_date=end_date, bulk=bulk)
    etl.spark = spark
    etl()
    spark.stop()
