"""
Sync cust_collections_acct from MySQL to Hudi via Spark Connect.
"""
import sys
import os
...
class StgCustCollectionsAcctEtl(MySqlEtl):
    src_db = "usr_skyee_mw"
    src_tbl = "cust_collections_acct"
    dst_db = "usr_skyee_mw"
    dst_tbl = "stg_cust_collections_acct"
    path = "/user/hive/warehouse/usr_skyee_mw.db/stg_cust_collections_acct"
    id = "ID"
    ts = "LST_UPD_TIME"
...
def main(
    ...
    etl = StgCustCollectionsAcctEtl(url=url, start_date=start_date, end_date=end_date, bulk=bulk)
    etl.spark = spark
    etl()
    spark.stop()
