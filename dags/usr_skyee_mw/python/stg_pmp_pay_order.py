"""
Sync pmp_pay_order from MySQL to Hudi via Spark Connect.
"""
import sys
import os
...
class StgPmpPayOrderEtl(MySqlEtl):
    src_db = "usr_skyee_mw"
    src_tbl = "pmp_pay_order"
    dst_db = "usr_skyee_mw"
    dst_tbl = "stg_pmp_pay_order"
    path = "/user/hive/warehouse/usr_skyee_mw.db/stg_pmp_pay_order"
    id = "PAY_ORDER_ID"
    ts = "LST_UPD_TIME"
...
def main(
    ...
    etl = StgPmpPayOrderEtl(url=url, start_date=start_date, end_date=end_date, bulk=bulk)
    etl.spark = spark
    etl()
    spark.stop()
